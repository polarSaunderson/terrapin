remove_incomplete_years <- function(x,
                                    daily = FALSE) {
  #' Subset a SpatRaster to exclude layers whose date is not matched in all
  #' years
  #'
  #' @description This function is perhaps easiest to explain with examples.
  ...
  #'
  #'   *Example 1*: A dataset runs continuously from November 1980 to December
  ...
  #'
  #'   *Example 2*: A dataset runs continuously from October 1980 to November
  ...
  #'
  #'   *Example 3*: Be careful with the concept of a "complete year" in this
  ...
  #' @seealso remove_incomplete_austral_summers()
  #'
  #' @export
  ...
  if (isFALSE(daily)) {
    ...
    # Which years have all the data?
    for (ii in xYears) {
      iiData   <- xDates[which(xDates$year == ii), ]
      iiDays   <- paste(sprintf("%02d", iiData$month),
                        sprintf("%02d", iiData$day),
                        sep = "-") |>
        unique() |> sort()                         # unique year ii dates
  ...
  # Subset
  xSubset <- subset_by_year(x, incYears)
